...
class Restore Extends Base\RestoreBase{
	...
	public function processConfigs($configs){
		...
		foreach ($configs['managedCerts'] as $cert) {
			try {
				$this->certman2->saveCertificate($cert['caid'], $cert['basename'], $cert['description'], $cert['type'], $cert['additional']);
			} catch (\Exception $e) {
		...
		foreach ($configs['managedCSRs'] as $csr) {
			$this->certman2->saveCSR($csr);
		}
		foreach ($configs['dtlsOptions'] as $dtls) {
			$dtls['certificate'] = $dtls['cid'];
			$this->certman2->addDTLSOptions($dtls['id'], $dtls);
		}
		return $this;
	}
